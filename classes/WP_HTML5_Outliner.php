...
class WP_HTML5_Outliner {
	...
	public static function add_to_toolbar( $wp_admin_bar ) {
		...
		// Adds an 'HTML5 Outliner' text node to the Toolbar. 
		$wp_admin_bar->add_node( [
			'id'    => __NAMESPACE__ . '-outliner-toolbar-node',
			'title' => __( 'HTML5 Outliner', 'wph50'),
	...
	private static function make_toolbar_html() {
		 ...
		 */
		$notice = sprintf( '<div class="%2$s-notice">%1$s</div>',
		...
		if ( self::$page ) {
			...
			$outline = ( new HTML5_Outline( self::$page ) )->render_outline();

		}

		if ( empty( $outline ) ) {

			return sprintf( '<div class="%2$s-wrapper">%1$s</div>',
							 $notice,
							 __NAMESPACE__ 
				   );
		...
		return sprintf( '<div class="%9$s-wrapper">
						   <div id="%9$s-popper" title="%1$s" aria-hidden="true">
						   </div><!-- #%9$s-popper -->
						   <div id="%9$s-outline-toggle" aria-hidden="true">
						     <a href="#%9$s-structural" id="%9$s-show-s" class="%9$s-active" 
							    title="%2$s">%3$s</a>
							 &nbsp; | &nbsp;
							 <a href="#%9$s-h-level" id="%9$s-show-hl" 
							    title="%2$s">%4$s</a>
